...
public class BPlusTree<K extends Comparable<K>, V> {
    ...
    //插入
    public void insert(K value, V key) {
        if (key == null)
            return;
        Node<K, V> K = this.root.insert(value, key);
        if (K != null) {
            this.root = K;
        }
        this.left = (LeafNode<K, V>) this.root.refreshLeft();
...
//        System.out.println();
    ...
    abstract class Node<K extends Comparable<K>, V> {
        ...
        //插入
        abstract Node<K, V> insert(K value, V key);

        abstract LeafNode<K, V> refreshLeft();
    }
...
//            System.out.println("非叶子节点查找key: " + this.keys[i]);
         ...
         * @param key
         */
        Node<K, V> insertNode(Node<K, V> node1, Node<K, V> node2, K key) {
...
//                System.out.println("非叶子节点,插入key: " + node1.keys[node1.number - 1] + " " + node2.keys[node2.number - 1] + ",新建父节点");
                ...
                BPlusNode<K, V> tempBPlusNode = new BPlusNode<>();
            ...
            BPlusNode<K, V> parentNode = (BPlusNode<K, V>) this.parent;
