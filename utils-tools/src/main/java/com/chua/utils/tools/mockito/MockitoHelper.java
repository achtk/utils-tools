package com.chua.utils.tools.mockito;
...
public class MockitoHelper {
     ...
     * @return
     */
    public static <A, B> List<Map<A, B>> batchData(int count, Class<A> stringClass, Class<B> objectClass) {
        List<Map<A, B>> result = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            Map<A, B> item = new HashMap<>(1);
            item.put(getValueByClass(stringClass), getValueByClass(objectClass));
            result.add(item);
        }
        return result;
    }
     ...
     * @return
     */
    public static List<HashMultimap> batchHashMultimapData(int count) {
        List<HashMultimap> result = new ArrayList<>(count);
        for (int i = 0; i < count; i++) {
            HashMultimap<String, Object> item = HashMultimap.create();
            String valueByClass = getValueByClass(String.class);
            String valueByObject = getValueByClass(String.class);
            item.put(valueByClass, valueByObject);
            result.add(item);
        }
        return result;
    }
    ...
    /**
     * 按类型生成数据
     * @param objectClass 基础类型
     * @param <T>
     * @return
     */
    private static <T> T getValueByClass(Class<T> objectClass) {
        String simpleName = objectClass.getSimpleName();
        if(Number.class.isAssignableFrom(objectClass)) {
            return (T) getNumber(10);
        }
