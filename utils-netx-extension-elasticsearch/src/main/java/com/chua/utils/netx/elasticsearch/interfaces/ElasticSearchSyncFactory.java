 ...
 */
@Slf4j
public class ElasticSearchSyncFactory implements IElasticSearchSyncFactory {
    ...
    public boolean createIndex(IndexConfig indexConfig) {
        try (JestClient jestClient = this.jestClientFactory.getObject()) {
            ...
            try {
                ...
                log.debug("索引:{} - 创建成功!", indexConfig.getIndex());
    ...
    public boolean deleteIndex(IndexConfig indexConfig) {
        try (JestClient jestClient = this.jestClientFactory.getObject()) {
            ...
            DeleteIndex deleteIndex =
                    new DeleteIndex.Builder(indexConfig.getIndex())
    ...
    public boolean deleteDocument(IndexConfig indexConfig) {
        try (JestClient jestClient = this.jestClientFactory.getObject()) {
            ...
            try {
                JestResult execute = jestClient.execute(delete);
    ...
    public List<IndexConfig> bulkAddDocument(List<IndexConfig> indexConfigs) throws IOException {
        if (null != indexConfigs) {
            List<IndexConfig> result = new ArrayList<>(indexConfigs.size());
            ...
            if (execute.isSucceeded() && log.isDebugEnabled()) {
                log.debug("批量上传索引存在失败: {}", execute.getErrorMessage());
                List<BulkResult.BulkResultItem> bulkItemResponse = execute.getFailedItems();
                for (BulkResult.BulkResultItem bulkResultItem : bulkItemResponse) {
                    log.debug(bulkResultItem.id + ":" + bulkResultItem.index + ":" + bulkResultItem.errorReason);
                }
            }
            return result;
        }
        ...
        if (null != indexConfigs) {
            ...
            try (JestClient jestClient = this.jestClientFactory.getObject()) {
                ...
                for (IndexConfig indexConfig : indexConfigs) {
                    try {
                        bulk.addAction(
                                new Update.Builder(indexConfig.getDoc())
            ...
            if (execute.isSucceeded() && log.isDebugEnabled()) {
                log.debug("批量更新索引存在失败: {}", execute.getErrorMessage());
                List<BulkResult.BulkResultItem> bulkItemResponse = execute.getFailedItems();
                for (BulkResult.BulkResultItem bulkResultItem : bulkItemResponse) {
                    log.debug(bulkResultItem.id + ":" + bulkResultItem.index + ":" + bulkResultItem.errorReason);
                }
    ...
    public List<?> bulkDeleteDocument(List<IndexConfig> indexConfigs) throws IOException {
        if (null != indexConfigs) {
            ...
            if (execute.isSucceeded() && log.isDebugEnabled()) {
                log.debug("批量删除索引存在失败: {}", execute.getErrorMessage());
                List<BulkResult.BulkResultItem> bulkItemResponse = execute.getFailedItems();
                for (BulkResult.BulkResultItem bulkResultItem : bulkItemResponse) {
                    log.debug(bulkResultItem.id + ":" + bulkResultItem.index + ":" + bulkResultItem.errorReason);
                }
        ...
        try (JestClient jestClient = this.jestClientFactory.getObject()) {
            ...
            if (null != boolQueryBuilder) {
                ...
                try {
                    ...
                    return dealSearchResponse(execute);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    public List<Map<String, Object>> query(String dsl, String indexName, String indexType) throws Exception {
        try (JestClient jestClient = this.jestClientFactory.getObject()) {
            try {
                SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
                searchSourceBuilder.query(dsl);
    ...
    public <T>T getDocumentById(IndexConfig indexConfig, Class<T> entity) throws Exception {
        ...
        return execute.getSourceAsObject(entity);
    }
    ...
    @Override
    public void notAnalyzed(String indexName, String indexType, String field) {
        JestResult execute;
        try (JestClient jestClient = this.jestClientFactory.getObject()) {

            Properties properties = new Properties();
            properties.put("type", indexType);
            properties.put("index", "not_analyzed");

            Map<String, Object> params = new HashMap<>();
            params.put("properties", properties);
            params.put("type", "text");
            params.put("fielddata", true);

            PutMapping.Builder builder = new PutMapping.Builder(indexName, indexType, params);
