 ...
 */
@AllArgsConstructor
public class ShareDataSchema<T> extends AbstractSchema {
    ...
    private class CalciteTable extends AbstractTable implements ScannableTable {
        ...
        public RelDataType getRowType(RelDataTypeFactory relDataTypeFactory) {
            ...
            if (!(entity instanceof Map)) {
                ...
                value.stream().forEach((Consumer<? super T>) new Consumer<Map<String, Object>>() {
                    @Override
                    public void accept(Map<String, Object> stringObjectMap) {
                        stringObjectMap.forEach((key, value) -> {
                            if (!columns.contains(key)) {
                                columns.add(key.toUpperCase());
                                columnType.add(typeFactory.createSqlType(SqlTypeName.get(value.getClass().getName())));
                            }
                        });
                    }
                });
